package com.runssnail.monolith.dal;
...
import com.runssnail.monolith.dal.conf.MyBatisConfiguration;
import com.runssnail.monolith.dal.conf.MyBatisXMLConfigBuilder;
import com.runssnail.monolith.dal.route.DataSourceRouter;

/**
 * use {@link MyBatisXMLConfigBuilder} to create {@link Configuration},
 * 
 * copy from {@link SqlSessionFactoryBean}
 * 
 * @author zhengwei
 */
public class MyBatisSqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean, ApplicationListener<ApplicationEvent> {
     ...
     * both. A transactional JDBC {@code Connection} for this {@code DataSource} will be provided to application code
     ...
     * meant for testing so that mock SqlSessionFactory classes can be injected. By default,
     ...
     * {@code SpringManagedTransactionFactory} should be appropriate for all cases: be it Spring transaction management,
     * EJB CMT or plain JTA. If there is no active transaction, SqlSession operations will execute SQL statements
     * non-transactionally. <b>It is strongly recommended to use the default {@code TransactionFactory}.</b> If not
     * used, any attempt at getting an SqlSession through Spring's MyBatis framework will throw an exception if a
     ...
     * {@code XMLConfigBuilder} API to build a {@code SqlSessionFactory} instance based on an Reader.
     *
    ...
    protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
        ...
        if (this.configLocation != null) {
            xmlConfigBuilder = new MyBatisXMLConfigBuilder(this.configLocation.getInputStream(), null,
                                                           this.configurationProperties, this.router);
            configuration = xmlConfigBuilder.getConfiguration();
        } else {
            if (logger.isDebugEnabled()) {
                logger.debug("Property 'configLocation' not specified, using default MyBatis Configuration");
            }
            configuration = new MyBatisConfiguration(router);
        ...
        if (this.transactionFactory == null) {
            this.transactionFactory = new com.runssnail.monolith.dal.transaction.SpringManagedTransactionFactory();
        }
